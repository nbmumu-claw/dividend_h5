...
const VERSIONS = [
  {
    ...
    date: '2026-06',
    items: [
      '新增账号系统：邮箱注册/登录/验证码登录，多设备数据自动云同步（改动即传、登录恢复），换设备不再手动导出导入',
      '「我的」页新建账户区：可改名、退出确认；清空自选强化提示（仅清当前账户）且不再误删持仓',
      'PC 端日历/收益/矩阵/持仓详情/设置类子页统一居中约束，大屏不拉伸',
    ],
  },
  {
    version: 'v2.2',
    date: '2026-06',
    items: [
      '新增分红到期提示：持仓标的临近股权登记日（前 1 天起至后 5 天）自动在自选页顶部与持仓详情提示，一键确认即录入分红（登记日、登记日持仓股数自动填充），不需要的可忽略',
      '港股分红改为按财年统计（末期息归入对应财年，与 A 股口径一致）：决策矩阵历史分红、网格页添加标的预填股息的同比与近 5 年均每股派息更准确',
      '网格页现价时间改为显示行情时间（股价对应时刻，非刷新时刻），盘后显示收盘时刻',
    ],
  },
  {
    version: 'v2.1',
    date: '2026-06',
    items: [
      '修复取消自选会误删持仓：发现页对有持仓 / 交易记录的标的禁止直接取消自选（提示先清空记录）；自选页移除有持仓标的时明确警示将一并删除交易记录',
    ],
  },
  {
    version: 'v2.0',
    ...
    items: [
      ...
      '网格页新增「添加标的」：搜索即自动填板块 / 每股股息，「其他」兜底板块，重复提示；支持港股（HK$ 计价）',
      ...
      '交易手续费改为按账户独立设置（不同账户费率可不同，切换账户自动按各自费率重算成本）',
    ],
  },
  {
    version: 'v1.9',
