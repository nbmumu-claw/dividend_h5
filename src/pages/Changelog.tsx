...
const VERSIONS = [
  {
    ...
    date: '2026-06',
    items: [
      '基金支持完善：发现页可添加场内基金（ETF / LOF）与场外开放式基金，搜索选中自动识别并勾选场内 / 场外，分别按实时价 / 每日净值计价',
      '基金分红自动预填：优先取 2025 全年派现之和（与「25 年度股息」口径一致），决策矩阵历史分红也支持基金',
      '「红利ETF」板块改名「红利基金」，同时容纳场内 ETF 与场外基金（老用户数据自动迁移，原有标的不丢）',
      '分红日历：分红股数改按「股权登记日当时持仓」计算（不再用当前总股数），每张分红卡标注「已记录 / 未记录」',
      '多设备云同步修复：解决个别账号重复存档、偶发「刷新后回到旧数据」（同步写入串行化 + 登录时取最新并自愈）',
      '性能优化：页面按需加载，首屏更快（收益页图表不再拖慢其它页面）',
    ],
  },
  {
    version: 'v2.4',
    date: '2026-06',
    items: [
      '决策矩阵新增「模拟加仓」：从现价或股息率档位逐档加仓，推演加仓后的摊薄成本；步长（0.25%/0.5%）、加档、每档股数可调，并显示每档投入金额。纯推演，不生成交易、不影响真实持仓',
      '发现页支持添加场外开放式基金：按最新净值计价，计入自选与收益统计（基金分红手动填写）',
      '收益页「持仓市值」「成本金额」可点开明细，按 美股 / AH股 分组显示各标的金额与占比',
      '设置新增「美股是否纳入收益统计」开关（默认纳入），关闭后收益页不计入美股',
      '网格页板块支持折叠（按筛选项独立记忆）、一键全部折叠 / 全部展开，折叠后显示均息 / 最高息 / 达买点简介；自选页卡片支持折叠',
    ],
  },
  {
    version: 'v2.3',
