...
type ForecastResult = { code: string; name: string; annualDps: number; terminalDps: number | null; yieldRate: number | null; price: number | null; annualProfit: number; h1Profit: number; payout: number; payoutMedian: number; shares: number; shareSourceDate: string | null; interim: number | null; seasonality: Seasonality[]; payouts: DividendPayoutRecord[]; history: DividendYearRecord[]; interimExceedsModel: boolean }
...
export default function DividendForecastEngine() {
  ...
  const runFor = async (keyword: string) => {
    ...
    try {
      ...
      const payout = average(payoutRates), payoutMedian = median(payoutRates)
