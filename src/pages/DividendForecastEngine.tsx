...
type DividendCommitment = { code: string; startYear: number; endYear: number; minPayoutRatio?: number; minDps?: number; includesInterim: boolean; conditional: boolean; conditions: string[]; announcementDate: string; sourceUrl: string; sourceName: string }
...
function calculateAnnualDps(annualProfit: number, shares: number, payout: number, interimAnchor: number | null, commitment: DividendCommitment | null) {
  const effectivePayout = Math.max(payout, commitment?.minPayoutRatio ?? 0)
  ...
  const policyDpsFloor = commitment ? Math.max(commitment.minDps ?? 0, commitment.minPayoutRatio ? annualProfit * commitment.minPayoutRatio / shares : 0) : null
...
export default function DividendForecastEngine() {
  ...
  return <main className="forecast-page"><div className="forecast-shell">
    ...
    {result && <>
      ...
      {result.commitment && <section className={`forecast-policy ${result.policyApplied ? 'is-applied' : ''}`}><div><b>{result.policyApplied ? '分红承诺下限已纳入' : '分红承诺下限校验'}</b><span>{result.commitment.startYear}–{result.commitment.endYear} 年有效 · {result.commitment.includesInterim ? '全年口径，含中期息' : '全年口径'} · {result.commitment.conditional ? '条件性承诺' : '量化承诺'}</span></div><strong>26E 政策下限 {result.policyDpsFloor?.toFixed(3)} 元/股</strong><p>{result.commitment.conditions.join('；')}。<a href={result.commitment.sourceUrl} target="_blank" rel="noreferrer">查看原公告</a></p></section>}
