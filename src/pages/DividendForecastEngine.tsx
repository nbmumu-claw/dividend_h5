...
type ForecastResult = { code: string; name: string; annualDps: number; terminalDps: number | null; yieldRate: number | null; price: number | null; annualProfit: number; h1Profit: number; payout: number; payoutAverage: number; payoutMedian: number; payoutMethod: 'average' | 'median'; shares: number; shareSourceDate: string | null; interim: number | null; seasonality: Seasonality[]; payouts: DividendPayoutRecord[]; history: DividendYearRecord[]; interimExceedsModel: boolean }
...
export default function DividendForecastEngine() {
  ...
  const runFor = async (keyword: string) => {
    ...
    try {
      ...
      const payoutMethod = Math.max(...payoutRates) > 1 || Math.max(...payoutRates) - Math.min(...payoutRates) > .3 ? 'median' : 'average'
      const payout = payoutMethod === 'median' ? payoutMedian : payoutAverage
