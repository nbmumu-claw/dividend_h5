...
type DividendCommitment = { code: string; startYear: number; endYear: number; minPayoutRatio?: number; minDps?: number; minCashAmount?: number; modelEligible?: boolean; basis?: string; includesInterim: boolean; conditional: boolean; conditions: string[]; announcementDate: string; sourceUrl: string; eastmoneySourceUrl?: string; sourceName: string }
...
function commitmentRule(commitment: DividendCommitment) {
  const rules = [
    commitment.minPayoutRatio !== undefined ? `现金分红不低于${commitment.basis || '归母净利润'}的${(commitment.minPayoutRatio * 100).toFixed(0)}%` : null,
    commitment.minDps !== undefined ? `每股不低于${commitment.minDps.toFixed(2)}元` : null,
    commitment.minCashAmount !== undefined ? `现金分红总额不低于${(commitment.minCashAmount / 1e8).toFixed(0)}亿元` : null,
  ].filter(Boolean)
...
export default function DividendForecastEngine() {
  ...
  const runFor = async (keyword: string) => {
    ...
    setLoading(true); setError(''); setResult(null); setMatches([]); setPayoutChoice('auto'); setPayoutTipOpen(false)
    try {
      if (!/^\d{6}$/.test(code)) {
      ...
      const calculation = calculateAnnualDps(annualProfit, shares, payout, interimAnchor, response.dividendCommitment)
      ...
      setResult({ code, name: response.name || code, annualDps, terminalDps: interim === null ? null : Math.max(annualDps - interim, 0), yieldRate: price && price > 0 ? annualDps / price : null, price, annualProfit, h1Profit, payout, effectivePayout, appliedPayout, payoutAverage, payoutMedian, payoutLatest, payoutMethod, systemPayoutMethod: payoutMethod, shares, shareSourceDate: response.latestShare.REPORT_DATE || response.latestShare.NOTICE_DATE || null, interim, priorInterim, priorAnnualDps, profitDps, interimAnchor, usesInterimAnchor, commitment: response.dividendCommitment, policyDpsFloor, policyApplied, seasonality, payouts: [...payouts].sort((a, b) => a.year - b.year), history: (history?.records ?? []).filter(item => item.year >= 2023 && item.year <= 2025).sort((a, b) => a.year - b.year), interimExceedsModel: interim !== null && interim > annualDps })
  ...
  const selectPayout = (choice: 'auto' | PayoutMethod) => {
    ...
    setResult(current => {
      ...
      const payout = method === 'average' ? current.payoutAverage : method === 'median' ? current.payoutMedian : current.payoutLatest
      const calculation = calculateAnnualDps(current.annualProfit, current.shares, payout, current.interimAnchor, current.commitment)
      const { annualDps, effectivePayout, profitDps, policyDpsFloor, policyApplied, usesInterimAnchor } = calculation
      const appliedPayout = annualDps * current.shares / current.annualProfit
      return { ...current, payout, effectivePayout, payoutMethod: method, profitDps, annualDps, appliedPayout, policyDpsFloor, policyApplied, usesInterimAnchor, terminalDps: current.interim === null ? null : Math.max(annualDps - current.interim, 0), yieldRate: current.price && current.price > 0 ? annualDps / current.price : null, interimExceedsModel: current.interim !== null && current.interim > annualDps }
    })
  }
  ...
  return <main className="forecast-page"><div className="forecast-shell">
    ...
    {result && <>
      ...
      <section className="forecast-hero-grid" aria-label="预测结果"><article className="forecast-main-kpi"><span>26E 每股股息</span><strong>{result.annualDps.toFixed(3)}<small>元</small></strong><p>{result.terminalDps === null ? '中期息尚未公告' : `预计末期：${result.terminalDps.toFixed(3)} 元`}</p></article><article><span>26E 预期股息率</span><strong className="forecast-accent">{result.yieldRate === null ? '—' : percent(result.yieldRate)}</strong><p>现价：{result.price === null ? '未取得' : `${result.price.toFixed(2)} 元`}</p></article><article><span>全年归母净利润</span><strong>{billion(result.annualProfit)}</strong><p>26H1：{billion(result.h1Profit)}</p></article><article className="forecast-payout-kpi"><span>26E 推算派息率</span><strong>{percent(result.appliedPayout)}</strong><p>{result.usesInterimAnchor ? `中期息锚定；历史基准 ${percent(result.payout)}` : `历史基准：23–25 年${result.payoutMethod === 'latest' ? '最近一年' : result.payoutMethod === 'median' ? '中位数' : '平均值'}`}</p><div className="forecast-payout-quick-switch"><button type="button" className={result.payoutMethod === 'average' ? 'active' : ''} onClick={() => selectPayout('average')}>平均 {percent(result.payoutAverage)}</button><button type="button" className={result.payoutMethod === 'median' ? 'active' : ''} onClick={() => selectPayout('median')}>中位 {percent(result.payoutMedian)}</button><button type="button" className={result.payoutMethod === 'latest' ? 'active' : ''} onClick={() => selectPayout('latest')}>最近 {percent(result.payoutLatest)}</button></div><div className={`forecast-info-tip ${payoutTipOpen ? 'is-open' : ''}`} tabIndex={0} aria-label="查看与切换近三年派息率" onClick={() => setPayoutTipOpen(open => !open)}>i<span role="tooltip"><b>近三年常规现金派息率</b>{result.payouts.map(item => <em key={item.year}>{item.year}<strong>{percent(item.payoutRatio / 100)}</strong></em>)}<div className="forecast-payout-switch"><button type="button" className={payoutChoice === 'auto' ? 'active' : ''} onClick={event => { event.stopPropagation(); selectPayout('auto'); setPayoutTipOpen(false) }}>系统默认 · {result.systemPayoutMethod === 'average' ? '平均值' : result.systemPayoutMethod === 'median' ? '中位数' : '最近一年'}</button><button type="button" className={payoutChoice === 'average' ? 'active' : ''} onClick={event => { event.stopPropagation(); selectPayout('average'); setPayoutTipOpen(false) }}>平均值 {percent(result.payoutAverage)}</button><button type="button" className={payoutChoice === 'median' ? 'active' : ''} onClick={event => { event.stopPropagation(); selectPayout('median'); setPayoutTipOpen(false) }}>中位数 {percent(result.payoutMedian)}</button><button type="button" className={payoutChoice === 'latest' ? 'active' : ''} onClick={event => { event.stopPropagation(); selectPayout('latest'); setPayoutTipOpen(false) }}>最近一年 {percent(result.payoutLatest)}</button></div><small>当前利润模型：<strong>{percent(result.effectivePayout)}</strong></small><small>26E 推算派息率：<strong>{percent(result.appliedPayout)}</strong></small></span></div></article></section>
      {result.commitment && <section className={`forecast-policy ${result.policyApplied ? 'is-applied' : ''}`}><div><b>{result.policyApplied ? '分红承诺下限已纳入' : result.commitment.modelEligible === false ? '分红承诺已记录，未直接套用' : '分红承诺下限校验'}</b><span>{result.commitment.startYear}–{result.commitment.endYear} 年有效 · {result.commitment.includesInterim ? '全年口径，含中期息' : '全年口径'} · {result.commitment.conditional ? '条件性承诺' : '量化承诺'}</span></div><strong>{result.commitment.modelEligible === false ? `口径：${result.commitment.basis || '公告口径'}` : `26E 政策下限 ${result.policyDpsFloor?.toFixed(3)} 元/股`}</strong><p><b>承诺内容：</b>{commitmentRule(result.commitment)}。{result.commitment.conditions.length > 0 ? `适用条件：${result.commitment.conditions.join('；')}。` : ''}<a href={result.commitment.eastmoneySourceUrl || result.commitment.sourceUrl} target="_blank" rel="noreferrer">{result.commitment.eastmoneySourceUrl ? '查看东方财富公告' : '查看原公告'}</a></p></section>}
      ...
      <section className="forecast-desktop-data"><div className="forecast-section-title"><div><h2>预测输入与计算过程</h2><p>所有展示值均为本次查询的原始输入或其直接计算结果。</p></div><span>单位：元 / 股 / 亿元</span></div><div className="forecast-data-layout"><div className="forecast-matrix"><div className="forecast-matrix-head"><span>利润季节性</span><span>H1 归母净利</span><span>全年归母净利</span><span>H1 / 全年</span></div><div className="forecast-matrix-row forecast-matrix-forecast"><b>2026E <small>预测</small></b><span>{billion(result.h1Profit)}</span><span>{billion(result.annualProfit)}</span><strong>{percent(median(result.seasonality.map(item => item.ratio)))}</strong></div>{result.seasonality.map(item => <div className="forecast-matrix-row" key={item.year}><b>{item.year}</b><span>{billion(item.h1Profit)}</span><span>{billion(item.annualProfit)}</span><strong>{percent(item.ratio)}</strong></div>)}<div className="forecast-matrix-row forecast-matrix-result"><b>中位数</b><span>—</span><span>—</span><strong>{percent(median(result.seasonality.map(item => item.ratio)))}</strong></div></div><div className="forecast-input-list"><div><span>已公告中期股息</span><b>{result.interim === null ? '尚未公告' : `${result.interim.toFixed(3)} 元/股`}</b></div><div><span>权益分派股本</span><b>{(result.shares / 1e8).toFixed(3)} 亿股<a className="forecast-source-link" href={`https://data.eastmoney.com/yjfp/detail/${result.code}.html`} target="_blank" rel="noreferrer">来源：东方财富分红方案</a></b></div><div><span>下半年修正系数</span><b>1.00 <small>无披露依据调整</small></b></div><div><span>预测末期股息</span><b>{result.terminalDps === null ? '待中期息公告后拆分' : `${result.terminalDps.toFixed(3)} 元/股`}</b></div></div></div><div className="forecast-formulas"><div className={`forecast-equation ${!result.usesInterimAnchor && !result.policyApplied ? 'is-selected' : ''}`}><b className="forecast-formula-label">利润模型</b><span>26H1 利润 <b>{billion(result.h1Profit)}</b></span><i>÷</i><span>季节性中位数 <b>{percent(median(result.seasonality.map(item => item.ratio)))}</b></span><i>×</i><span>有效派息率 <b>{percent(result.effectivePayout)}</b></span><i>÷</i><span>权益股本 <b>{(result.shares / 1e8).toFixed(3)} 亿股</b></span><i>=</i><strong>{result.profitDps.toFixed(3)} 元/股</strong><em>{!result.usesInterimAnchor && !result.policyApplied ? '已采用' : '参考'}</em></div>{result.interimAnchor !== null && <div className={`forecast-equation ${result.usesInterimAnchor && !result.policyApplied ? 'is-selected' : ''}`}><b className="forecast-formula-label">中期息同比锚定</b><span>25年全年股息 <b>{result.priorAnnualDps?.toFixed(3)} 元</b></span><i>×</i><span>26H1 中期息 <b>{result.interim?.toFixed(3)} 元</b></span><i>÷</i><span>25H1 中期息 <b>{result.priorInterim?.toFixed(3)} 元</b></span><i>=</i><strong>{result.interimAnchor.toFixed(3)} 元/股</strong><em>{result.usesInterimAnchor && !result.policyApplied ? '已采用（更保守）' : '参考'}</em></div>}{result.commitment && <div className={`forecast-equation ${result.policyApplied ? 'is-selected' : ''}`}><b className="forecast-formula-label">政策下限</b><span>政策派息率 <b>{result.commitment.minPayoutRatio ? percent(result.commitment.minPayoutRatio) : '—'}</b></span><i>×</i><span>26E 归母净利 <b>{billion(result.annualProfit)}</b></span><i>÷</i><span>权益股本 <b>{(result.shares / 1e8).toFixed(3)} 亿股</b></span>{result.commitment.minDps !== undefined && <><i>，至少</i><span><b>{result.commitment.minDps.toFixed(3)} 元/股</b></span></>}<i>=</i><strong>{result.policyDpsFloor?.toFixed(3)} 元/股</strong><em>{result.policyApplied ? '已纳入' : '下限校验'}</em></div>}<p>{result.policyApplied ? `已纳入 ${result.commitment?.sourceName} 的有效期内量化下限；该承诺${result.commitment?.conditional ? '存在公告列明的适用条件，' : ''}不代表公司分红承诺。` : result.usesInterimAnchor ? `利润模型为 ${result.profitDps.toFixed(3)} 元/股；中期息同比结果为 ${result.interimAnchor?.toFixed(3)} 元/股，当前采用更保守的中期息同比结果。` : '选择利润模型：未触发中期息锚定或政策下限高于模型的条件。'}</p></div></section>
