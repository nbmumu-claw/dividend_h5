...
type ForecastResult = { code: string; name: string; annualDps: number; terminalDps: number | null; yieldRate: number | null; price: number | null; annualProfit: number; h1Profit: number; payout: number; appliedPayout: number; payoutAverage: number; payoutMedian: number; payoutLatest: number; payoutMethod: PayoutMethod; systemPayoutMethod: PayoutMethod; shares: number; shareSourceDate: string | null; interim: number | null; priorInterim: number | null; priorAnnualDps: number | null; profitDps: number; interimAnchor: number | null; usesInterimAnchor: boolean; seasonality: Seasonality[]; payouts: DividendPayoutRecord[]; history: DividendYearRecord[]; interimExceedsModel: boolean }
...
export default function DividendForecastEngine() {
  ...
  const selectPayout = (choice: 'auto' | PayoutMethod) => {
    ...
    setResult(current => {
      ...
      const profitDps = current.annualProfit * payout / current.shares
      const usesInterimAnchor = current.interimAnchor !== null && current.interimAnchor < profitDps * .9
      const annualDps = usesInterimAnchor ? current.interimAnchor! : profitDps
      const appliedPayout = annualDps * current.shares / current.annualProfit
      return { ...current, payout, payoutMethod: method, profitDps, annualDps, appliedPayout, usesInterimAnchor, terminalDps: current.interim === null ? null : Math.max(annualDps - current.interim, 0), yieldRate: current.price && current.price > 0 ? annualDps / current.price : null, interimExceedsModel: current.interim !== null && current.interim > annualDps }
    })
  }
  ...
  return <main className="forecast-page"><div className="forecast-shell">
    ...
    <div className="forecast-rule"><b>本页规则</b><span>先用利润模型估算股息；若已公告两年中期息，且“上年全年股息 × 中期息同比”较利润模型低 10% 以上，则采用更保守的中期息锚定结果，并反推 26E 派息率。缺少任一可审计输入即暂不覆盖。</span></div>
