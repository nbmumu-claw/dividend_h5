...
export default function DividendForecastEngine() {
  const [query, setQuery] = useState('000423'), [result, setResult] = useState<{ name: string; dps: number; yield: number | null; profit: number; payout: number; shares: number; interim: number } | null>(null), [error, setError] = useState(''), [loading, setLoading] = useState(false)
  const run = async () => {
    const code = query.trim(); if (!/^\d{6}$/.test(code)) { setError('请输入 6 位 A 股代码'); return }
    setLoading(true); setError(''); setResult(null)
    try {
      const [remote, payouts, prices] = await Promise.all([fetch(`${gateway}?action=forecastData&code=${code}`).then(r => r.json() as Promise<Remote>), fetchDividendPayouts(code), fetchStockPrices([{ code }], true)])
      ...
      setResult({ name: remote.name, dps, yield: price ? dps / price : null, profit, payout, shares: shares / 1e8, interim })
    } catch (e) { setError(e instanceof Error ? e.message : '查询失败') } finally { setLoading(false) }
  }
  return <main className="forecast-page"><header><small>DIVIDEND FORECAST ENGINE · LIVE</small><h1>分红预测引擎</h1><p>实时调用财报、权益股本、派息率与行情接口。</p></header><section className="forecast-search"><input value={query} onChange={e => setQuery(e.target.value)} /><button onClick={run}>{loading ? '查询中…' : '查询'}</button></section>{error && <section>{error}</section>}{result && <section><h2>{result.name} · {query}</h2><div className="forecast-kpis"><div><small>26E 每股股息</small><b>{result.dps.toFixed(3)} 元</b></div><div><small>26E 预期股息率</small><b>{result.yield ? pct(result.yield) : '—'}</b></div><div><small>全年归母净利</small><b>{(result.profit / 1e8).toFixed(2)} 亿元</b></div><div><small>中期已公告股息</small><b>{result.interim.toFixed(3)} 元</b></div></div><p>常规派息率 {pct(result.payout)} · 权益股本 {result.shares.toFixed(3)} 亿股</p></section>}</main>
}
